...
class TestHistoricalData(unittest.TestCase):
    ...
    * Subscription of US Futures market data is REQUIRED for some tests.
    ...
    async def test_get_historical_ticks_0(self):
        ...
        * Request tick data for `BID_ASK`.
        ...
        result = await self._bridge.get_historical_ticks(
            ...
            end=self._end, data_type=datatype.HistoricalTicks.BID_ASK,
            attempts=1
        )
    ...
    @utils.async_test
    async def test_get_historical_ticks_1(self):
        """Test function `get_historical_ticks`.
        ...
        * Request tick data for `MIDPOINT`.
        ...
        result = await self._bridge.get_historical_ticks(
            contract=sample_contracts.gbp_usd_fx(), start=self._start,
            end=self._end, data_type=datatype.HistoricalTicks.MIDPOINT,
            attempts=1
        )
    ...
    @utils.async_test
    async def test_get_historical_ticks_2(self):
        """Test function `get_historical_ticks`.

        * Request tick data for `TRADES`.
        """
        result = await self._bridge.get_historical_ticks(
            contract=sample_contracts.us_future(), start=self._start,
            end=self._end, data_type=datatype.HistoricalTicks.TRADES,
            attempts=1
        )
    ...
    @utils.async_test
    async def test_get_historical_ticks_err_0(self):
        """Test function `get_historical_ticks`.

        * Expect `ValueError` as argument `start` or `end` contains timezone
          info.
        """
        with self.assertRaises(ValueError):
            await self._bridge.get_historical_ticks(
                contract=sample_contracts.gbp_usd_fx(),
                start=_global.TZ.localize(dt=self._start),
                end=self._end, data_type=datatype.HistoricalTicks.BID_ASK,
                attempts=1
            )
        ...
        with self.assertRaises(ValueError):
            await self._bridge.get_historical_ticks(
                contract=sample_contracts.gbp_usd_fx(),
                start=self._start, end=_global.TZ.localize(dt=self._end),
                data_type=datatype.HistoricalTicks.BID_ASK, attempts=1
            )

    @utils.async_test
    async def test_get_historical_ticks_err_1(self):
        """Test function `get_historical_ticks`.

        * Expect `ValueError` as value of `start` is earlier than the earliest
          available datapoint.
        """
        with self.assertRaises(ValueError):
            await self._bridge.get_historical_ticks(
    ...
    @utils.async_test
    async def test_get_historical_ticks_err_2(self):
        """Test function `get_historical_ticks`.

        * Expect `ValueError` as value of `end` is earlier than `start`
        """
        with self.assertRaises(ValueError):
            await self._bridge.get_historical_ticks(
                contract=sample_contracts.gbp_usd_fx(),
    ...
    @utils.async_test
    async def test_get_historical_ticks_err_3(self):
        """Test function `get_historical_ticks`.

        * Expect `ValueError` as value of `attempts` < 1 and != -1.
        """
        with self.assertRaises(ValueError):
            await self._bridge.get_historical_ticks(
                contract=sample_contracts.gbp_usd_fx(),
                start=self._start, end=self._end,
                data_type=datatype.HistoricalTicks.BID_ASK, attempts=-2
            )

    @utils.async_test
    async def test_get_historical_ticks_err_4(self):
        """Test function `get_historical_ticks`.

        * Expect `IBError` due to invalid `Contract` passed in.
        """
        with self.assertRaises(error.IBError):
            await self._bridge.get_historical_ticks(
